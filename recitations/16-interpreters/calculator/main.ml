...
(** [lex s] converts [s] to a sequence of tokens *)
let tokens (s : string) : Parser.token list =
  let tokens = ref [] in
  let lexbuf = (Lexing.from_string s) in
  let current = ref (Lexer.read lexbuf) in
  let _ = tokens := !current :: !tokens in
  while !current <> Parser.EOF do current := (Lexer.read lexbuf); tokens := !current :: !tokens done; List.rev !tokens 
